    ...
    namespace com
    {
        ...
        {    
            ...
            std::vector<uint8_t> Ipv4EndpointOption::Payload() const
            {
                ...
                _result.push_back(0x00);
                _result.push_back(static_cast<uint8_t>(mL4Proto));
                ...
                uint16_t port)
            {
                ...
                std::cout << "_firstOctet " << _firstOctet <<std::endl;
            ...
            std::unique_ptr<Ipv4EndpointOption> Ipv4EndpointOption::Deserialize(
                ...
                std::size_t &offset, OptionType type, bool discardable)
            {
                ...
                auto _protocol = static_cast<Layer4ProtocolType>(payload.at(offset++));
                ...
                switch (type)
                {
                    case OptionType::IPv4Endpoint:
                        return CreateUnitcastEndpoint(discardable, _ipAddress, _protocol, _port);

                    case OptionType::IPv4Multicast:
                        return CreateMulticastEndpoint(discardable, _ipAddress, _port);

                    case OptionType::IPv4SdEndpoint:
                        return CreateSdEndpoint(discardable, _ipAddress, _protocol, _port);

                    default:
                        throw std::out_of_range("The option type does not belong to IPv4 endpoint option series.");
                }
            }
        }
    }
