        ...
        ssize_t Send(const std::array<uint8_t, N> &buffer) const noexcept
        {
            ssize_t _result =
                send(
                    ...
                    buffer.data(),
                    N,
                    MSG_NOSIGNAL); //By using MSG_NOSIGNAL, you can avoid the default behavior of terminating the process on a broken pipe and handle the situation more gracefully in your code.
