...
namespace ara 
{
    namespace com
    {
        namespace proxy
        {    
                SwNameVersionProxy::HandleType::HandleType(InstanceIdentifier id,int16_t p): identifier{id},
                                                                                            port_no{p}
                {}

                void SwNameVersionProxy::HandleType::setId(InstanceIdentifier id)
                {   identifier=id;  }

                void SwNameVersionProxy::HandleType::setPort(int16_t p)
                {   port_no=p;  }

                int16_t SwNameVersionProxy::HandleType::getPort()const
                {   return port_no; }

                const InstanceIdentifier& SwNameVersionProxy::HandleType::GetInstanceId()const
                {   return identifier;  }

                void SwNameVersionProxy::HandleType::setRequester(FieldServiceRequester *r)
                {   requester = r;  }

                FieldServiceRequester* SwNameVersionProxy::HandleType::getRequester()
                { return requester; }  


                ServiceHandleContainer<SwNameVersionProxy::HandleType> SwNameVersionProxy::findSerivce(InstanceIdentifier id)
                {
                ...
                SwNameVersionProxy::SwNameVersionProxy (HandleType &handle)
                {
                ...
                void SwNameVersionProxy::subscribe(size_t maxSampleCount)
                {
                    requester->fieldClient->Subscribe(maxSampleCount);
                }

                helper::SubscriptionState SwNameVersionProxy::GetSubscriptionState() const
                {
                    return requester->fieldClient->GetSubscriptionState();
                }

                std::future<bool> SwNameVersionProxy::set(std::vector<uint8_t> &data)
                {
                    return requester->fieldClient->set(data);
                }

                std::future<bool> SwNameVersionProxy::get(std::vector<uint8_t> &data)
                {
                    return requester->fieldClient->get(data);
                }

                void SwNameVersionProxy::printSubscriptionState()
                {
                    requester->fieldClient->printCurrentState();
                }

                void SwNameVersionProxy::SetReceiveHandler(Handler h)
                {
                    requester->fieldClient->SetReceiveHandler(h);
                }

                void SwNameVersionProxy::UnsetReceiveHandler()
                {
                ...
                SwNameVersionProxy::~SwNameVersionProxy()
                {
                    delete requester;
                }
        }
